...
class Model extends EventEmitter {
  ...
  async exportCsv(sourceName, filepath) {
    ...
    let rows = await pg.query(`SELECT * FROM ${s.table_view} WHERE source_name = $1`, [sourceName]);
    rows = rows.map(row => {
      delete row.source_name
      return row;
    });
    
    await csv.export(filepath, rows, revision, uid);
  ...
  async analyzeUpdate(filename, sheetname, data, revision) {
    let s = await source.getSource(filename, sheetname);
    if( s === null ) return {error: new Error('Unknown source: '+source.getSourceName(filename, sheetname))};

    // TODO: check revision id
    if( s.revision !== revision ) return {error: new Error(`Invalid revision: ${revision}, database is currently at ${s.revision}. please export source data again`)};

    let uid = config.uids[s.table_view];
    if( !uid ) return {error: new Error(`Unknown table ${s.table_view} pk: Please specify in ${config.TABLES.PK} table`)};
