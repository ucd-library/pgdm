...
class Model extends EventEmitter {
  ...
  async exportAll(folderPath) {
    ...
    for( let sheet of sheets ) {
      await fs.mkdirp(path.join(folderPath, sheet.table_view));
  ...
  _appendInserts(cmds, data, table, customFn, options={}) {
    ...
    for( let row of data ) {
      ...
      if( c % this.BATCH_SIZE === 0 ) {
        ...
        cmds.push(() => this.emit(
          `${options.type || 'insert'}-update`, 
          {total: options.total || data.length, current: row, op: options.op || 'insert'}
          ));
        stmt = '';
      }
    ...
    if( stmt ) {
      cmds.push(() => this.emit(`${options.type || 'insert'}-update`, {total: options.total || data.length, current: c-1}));
      cmds.push({stmt, params:[], row: c, table});
    }
  ...
  async update(analyzeInfo) {
    ...
    for( let row of analyzeInfo.updates ) {
      ...
      if( count % this.BATCH_SIZE === 0 ) {
        ...
        cmds.push(() => this.emit('update-update', {total, current: row, op: update}));
        stmt = '';
      }
    }
    ...
    if( stmt ) {
      ...
      cmds.push(() => this.emit('update-update', {total, current: count, op: update}));
    }

    
    // INSERT
    count = this._appendInserts(cmds, analyzeInfo.inserts, analyzeInfo.source.table_view, (keys, values) => {
      keys.push('source_name');
