...
class Model extends EventEmitter {
  ...
  _appendInserts(cmds, data, table, customFn) {
    let stmt = '';
    let c = 1;
    ...
    for( let row of data ) {
      ...
      if( c % this.BATCH_SIZE === 0 ) {
        ...
        cmds.push(() => this.emit('insert-update', {total: data.length, current: row}));
        stmt = '';
      }
    ...
    if( stmt ) {
      cmds.push(() => this.emit('insert-update', {total: data.length, current: c-1}));
      cmds.push({stmt, params:[], row: c, table});
    }
  ...
  async delete(sourceName) {
    ...
    try {
      await pg.query('BEGIN');
      let stmt = `DELETE from ${table} where source_id = $1`;
  ...
  async replace(sourceName, filename, sheetname, data) {
    ...
    try {
      ...
      for( let row of data ) {
        ...
        await this.pgQueryWrapper('replace', stmt, params, errors, {row: c, table});

        this.emit('replace-update', {total: data.length, current: c++});
      }
  ...
  async pgQueryWrapper(type, stmt, params, info) {
    try {
      await pg.query(stmt, params);
    } catch(e) {
      e = new Error(e, stmt, params, info);
      errors.push(e);
      this.emit(type+'-update', e);
    }
  }
